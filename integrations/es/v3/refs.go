// refs.go 定义 es integration 的命名引用 wrapper 与 binding 注册辅助。
package es
...
// ClientNamer 允许 marker 类型自定义 `ClientRef` 对应的 ES client 名称。
type ClientNamer interface {
	ESClientName() string
}

// DefaultClient 是默认 ES client 的 marker 类型。
type DefaultClient struct{}

// ESClientName 返回默认 client 名称。
func (DefaultClient) ESClientName() string { return "default" }

// InjectClient 用于在 handler 参数中注入默认 ES Client。
type InjectClient struct {
	*Client
}

// ClientRef 用于在 handler 参数中注入“按名字区分”的 ES Client。
type ClientRef[N any] struct {
	*Client
	_ *N
}

// ClientToken 返回按 name 区分的 ES client token。
func ClientToken(name string) string {
	if name == "" {
		name = "default"
	}
	return "es:client:" + name
}

// RegisterBindings 注册 ES 相关的命名注入 wrapper。
...
func RegisterBindingsCompat() {
	registerBindings(registry.GlobalWithUsage("integrations/es.RegisterBindingsCompat"))
}

func registerBindings(reg *registry.Registry) {
	reg.RegisterBindings(runtime.NewBindingResolvers(
		registration("ESInjectClient", coreintegration.MatchNamedWrapper(packagePath(), "InjectClient"), resolveInjectClient),
		registration("ESClientRef", coreintegration.MatchNamedWrapper(packagePath(), "ClientRef"), resolveClientRef),
	)...)
}
...
func resolveNamedClient(ctx *runtime.HandlerContext, name string) (*Client, error) {
	if ctx == nil || ctx.Container == nil {
		return nil, fmt.Errorf("es client binding requires request container")
	}
...
func clientNameFromWrapper(wrapperType reflect.Type) string {
	return coreintegration.ResolveMarkerName(wrapperType, "default", func(marker any) (string, bool) {
		...
		return namer.ESClientName(), true
	})
}
